...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, args):
        ...
        for k, v in kwargs.items():
            without_quote = remove_quotes(v)
            if without_quote != v:
                without_quote = without_quote.replace("_", " ")
                setattr(new_instance, k, without_quote)
            elif is_convertible_to_int(v):
                setattr(new_instance, k, int(v))
            elif is_convertible_to_float(v):
...
def remove_quotes(variable):
    ...
    if match:
        return match.group(1)  # Return the text inside the quotes
    else:
        return variable
